...
void Solver::Update(int action, OBS_TYPE obs) {
	...
	//model_->UpdateLocalBelief(belief()); // update local belief in the model with updated belief from solver
...
void Solver::UpdateRandomNum(double num) {
	belief_->setRandNum(num);
}
