...
void Belief::setRandNum(double num) {
	rand_num = num;
}
...
vector<State*> Belief::Sample(int num, vector<State*> particles,
	...
	for (int i = 0; i < num; i++) {
		...
		particle->weight = unit; // why keep the same weigtht (probability) ?
	...
	while (count < num || trial < 200 * num) {
		...
		for (int i = hstart; i < history.Size(); i++) {
			belief.model_->Step(*particle, Random::RANDOM.NextDouble(),
				history.Action(i), reward, obs); // history.observation(i) ?
...
ParticleBelief::ParticleBelief(vector<State*> particles, const DSPOMDP* model,
	...
	state_indexer_(NULL) {
		...
		//printf("initial procedure for initial belief");
...
void ParticleBelief::setRandNum(double num) {
	rand_num = num;
}
...
void ParticleBelief::Update(int action, OBS_TYPE obs) {
	...
	//printf("update process in a concrete particle belief class.\n");

	//printf("particles size: " + particles_.size());

	for (int i = 0; i < particles_.size(); i++) {
		State* particle = particles_[i];
		//bool terminal = model_->Step(*particle, Random::RANDOM.NextDouble(), action, reward, o);
		bool terminal = model_->Step(*particle, rand_num, action, reward, o);

		//printf("random num in belief update procedure: %5f \n", rand_num);
		...
		//printf("particle: " + particle->robot_position + "; weight: " + particle->weight);
	}
...
// 	OBS_TYPE o;
// 	// Update particles
// 	for (int i = 0; i <particles_.size(); i++) { // sampled or unsampled particle set of initial belief particle sets (n = 5000)
// 		State* particle = particles_[i];
