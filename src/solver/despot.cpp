...
ValuedAction DESPOT::Search() { // important procedure for planning
	...
	//printf("despot sampling particles");
	...
	// when construct the tree, the Step function is called by the Expand and Trail functions
	...
	printf("go into the search procedure of despot.\n");
	...
	ValuedAction astar = OptimalAction(root_);// focus here
	start = get_time_second();
	delete root_;
	...
	//printf("the optimal action is: %d \n", astar.action);
	return astar;
}
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		bool terminal = model->Step(*copy, streams.Entry(copy->scenario_id),
			qnode->edge(), reward, obs); // here, construct the tree, not need to execute the action actually.
...
void DESPOT::Update(int action, OBS_TYPE obs) {
	double start = get_time_second();
	//printf("Update process in despot solver.\n");
