...
bool Evaluator::RunStep(int step, int round) {
	printf("go into the runstep procedure.\n");
	...
	int action = solver_->Search().action; // the program stops here.!!!
...
/*	if (local_belief != NULL) {	
		*out_ << "-Local Belief at initial round is = " << endl;         //user-defined
		model_->PrintBelief(*local_belief, *out_);   //user-defined
	}
*/
	if (belief_ != NULL) {
		*out_ << "- Belief before updating = " << endl;
		model_->PrintBelief(*belief_, *out_);
		*out_ << endl;
	}
	

/*	Belief* local_belief_ = model_->GetLocalBelief();     //user-defined

	if (local_belief_ != NULL) {	
		*out_ << "-Local Belief is = " << endl;         //user-defined
		model_->PrintBelief(*local_belief_, *out_);   //user-defined
	} else {
		*out_ << "Local Belief is empty" << endl;
	}*/
...
//------------------------------------------------
	...
	if (!Globals::config.silence && out_) {
		*out_ << "- Selected Action = ";
		model_->PrintAction(action, *out_);
	}

	if (!Globals::config.silence && out_) {
		*out_ << "- Received Observation = ";
		model_->PrintObs(*state_, obs, *out_);
	}

	if (state_ != NULL) {
		if (!Globals::config.silence && out_) {
			*out_ << "- Resulting State:\n";
			model_->PrintState(*state_, *out_);
		}
	}


	/*if (model_->GetSimulatedBelief() != NULL) {
		*out_ << "-calculating simulated belief = " << endl;
		model_->PrintSimulatedBelief(*model_->GetSimulatedBelief());
	}*/
...
//------------------user modified-----------------------
	solver_->UpdateRandomNum(random_num);

	solver_->Update(action, obs); //update belief
	
	model_->UpdateLocalBelief(belief_); // update POMDP model's local belief with new belief

	//model_->UpdateBeliefStatus(belief_); //check the status of updated belief

	ReportStepReward();

	if (belief_ != NULL) {
		*out_ << "\n" << "- Belief after updating = " << endl;
		model_->PrintBelief(*belief_, *out_);
	}



/*	local_belief_ = model_->GetLocalBelief();     //user-defined
...
/*int Evaluator::UpdateBeliefStatus(Belief& belief) {
}*/
...
void POMDPEvaluator::InitRound() {
	...
	////////////////////////////////////////////////////////////////// user-defined block for debugging
	if (belief != NULL && out_) {
		model_->UpdateLocalBelief(belief);   // local belief initialization in the model                      
		*out_ << "Initial local belief: " << endl;
		model_->PrintBelief(*model_->GetLocalBelief(), *out_);
		*out_ << endl;
	} else {
		*out_ << "Initial belief is NULL" << endl;
	}
	//////////////////////////////////////////////////////////////////


	belief_ = belief;//user-customed for belief display

	//local_belief_ = belief; //user-customed for belief display
...
bool POMDPEvaluator::ExecuteAction(int action, double& reward, OBS_TYPE& obs) {
	random_num = random_.NextDouble();

	//printf("random num in ExecuteAction: %5f \n",random_num);
